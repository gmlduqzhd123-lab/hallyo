...
import { Code2, Plus, Trash2, Edit2, Upload, Loader2, Video, Globe, Camera, Code, Mail, User, Briefcase, X, ExternalLink, Link as LinkIcon, Users, PenTool } from 'lucide-react'
import { toast } from 'sonner'
import { addDeveloper, updateDeveloper, deleteDeveloper } from '@/app/actions/developers'
import DeveloperNotes from './developer-notes'
...
export default function DevelopersPage() {
  const [activeTab, setActiveTab] = useState<'intro' | 'notes'>('intro')
  ...
  return (
    <main className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-gradient-to-r from-violet-600 via-purple-500 to-fuchsia-500 p-6 md:p-8 rounded-[32px] shadow-lg shadow-purple-500/20 text-white relative overflow-hidden border border-white/20">
        ...
        {activeTab === 'intro' && ['admin', 'developer'].includes(userRole as string) && (
          <button
      ...
      {/* Tabs */}
      <div className="flex flex-wrap gap-2 md:gap-4 justify-start bg-white p-2 rounded-2xl shadow-sm border border-slate-100 max-w-fit mx-auto md:mx-0">
        <button
          onClick={() => setActiveTab('intro')}
          className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm md:text-base transition-all ${
            activeTab === 'intro'
      ...
      {activeTab === 'intro' ? (
        ...
        <DeveloperNotes userRole={userRole as string} />
      )}
    </main>
  )
}
